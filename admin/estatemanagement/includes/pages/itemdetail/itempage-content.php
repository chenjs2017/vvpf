					...
					**/
						...
						if((!empty($images) || !empty($featured_image[0]))){
							...
							$tabinside .= '<section role="itempagegallery" class="pf-itempage-gallery pf-itempage-elements">';
								...
								if(PFSAIssetControl('setup42_itempagedetails_featuredimage','','1') == 1 && !is_rtl()){
									$output .= "<li class='item'>";
									if ($di_lbox_v == 1) {$output .= "<a href='".$featured_image_orj[0]."' rel='prettyPhoto'>";}
								...
								if(!empty($images)){
									...
									if (is_rtl()) {
									    ...
									    $output .= "<li class='item'>";
									    if ($di_lbox_v == 1) {$output .= "<a href='{$image['full_url']}' rel='prettyPhoto'>";}
											...
											if(count($user_socials) > 0){
												$tabinside .= '<ul class="pf-itempage-sidebarinfo-social'.$css_text.' pf-row clearfix">';
													foreach ($user_socials as $keyx => $valuex) {
														$tabinside .= '<li class="pf-sociallinks-item '.$keyx.'  wpf-transition-all '.$col_text.'"><a href="'.$valuex.'" target="_blank"><i class="pfadmicon-'.pfsocialtoicon($keyx).'"></i></a></li>';
													}
												$tabinside .= '</ul>';
											}
												...
												if(count($user_socials) > 0){
													$tabinside .= '<ul class="pf-itempage-sidebarinfo-social'.$css_text.' pf-row clearfix">';
														foreach ($user_socials as $keyy => $valuey) {
															$tabinside .= '<li class="pf-sociallinks-item '.$keyy.'  wpf-transition-all '.$col_text.'"><a href="'.$valuey.'" target="_blank"><i class="pfadmicon-'.pfsocialtoicon($keyy).'"></i></a></li>';
														}
													$tabinside .= '</ul>';
												}
