...
//jschen
			...
			if($pfg_orderby != ''){
				...
				elseif ($pfg_orderby =='distance') {
					...
					if ($pfrandomize == 'yes') {
						if(isset($args['orderby'][$pfg_orderby])){unset($args['orderby'][$pfg_orderby]);}
						$args['orderby']['rand']='';
					}
		...
		if (isset($pfgetdata['manual_args']['search_prod_title'])) {
			$pfgetdata['manual_args']['search_prod_title'] = html_entity_decode($pfgetdata['manual_args']['search_prod_title']);
		}
		...
		/* Start: Coordinate Filter */
			if (empty($pfgetdata['manual_args'])) {
				...
				if ((!empty($sw) && !empty($sw2) && !empty($ne) && !empty($ne2))) {
					...
					$pfgetdata['manual_args']['pf_sw'] = $sw;
					$pfgetdata['manual_args']['pf_sw2'] = $sw2;
					$pfgetdata['manual_args']['pf_ne'] = $ne;
					$pfgetdata['manual_args']['pf_ne2'] = $ne2;
			
					$loop = new Pointfinder_WP_GeoQuery( $pfgetdata['manual_args'] );
				}else{
...
//										$ItemDetailArr['if_address'] = esc_html(get_post_meta( $pfitemid, 'webbupointfinder_items_address', true ));
													...
													/* Title and address area */
														...
														if (!empty($pfgetdata['manual_args'])) {
															if ($setup22_searchresults_showmapfeature == 1 && $setup42_searchpagemap_headeritem == 1) {
																$wpflistdata_output .= '<div class="pflist-subdetailcontainer pflist-subitem pfshowmapmain"><a data-pfitemid="'.$pfitemid.'" class="pfshowmaplink"><i class="pfadmicon-glyph-372"></i> '.esc_html__('SHOW ON MAP','pointfindert2d').'</a></div>';
															}
														}
												/* End: Detail Texts */
					...
					if ($infinite_scroll == 1) {
						...
						$(function(){
							...
							$(".pflistgridview'.$gridrandno_orj.'-content").imagesLoaded( function() {
